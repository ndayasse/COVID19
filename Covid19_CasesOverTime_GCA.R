...
#### Growth Curve Analysis of New Cases from Day 0: ######

# make a version (to get rid of NA's on) for GCA:
who.new.cum.cases.time.sel.nonzero.gca <- dplyr::select(who.new.cum.cases.time.sel.nonzero,
                                                        c(reporting_country_territory,
                                                          new_confirmed_cases,
                                                          Days_Count))
who.new.cum.cases.time.sel.nonzero.gca <- na.omit(who.new.cum.cases.time.sel.nonzero.gca)

# Re-calc Days_Count to start at 1 not 0 for the sake of GCA:
who.new.cum.cases.time.sel.nonzero.gca$Days_Count <- who.new.cum.cases.time.sel.nonzero.gca$Days_Count +1

# create 3rd-order polynomial in the range of Days_Count
t <- poly (unique(who.new.cum.cases.time.sel.nonzero.gca$Days_Count), 3)
# create variables ot1, ot2, ot3 corresponding to the orthogonal polynomial 
#   time terms and populate their values
#   with the Days_Count-appropriate orthogonal polynomial values:
who.new.cum.cases.time.sel.nonzero.gca[,paste("ot", 1:3, sep="")] <- 
  t[who.new.cum.cases.time.sel.nonzero.gca$Days_Count, 1:3]

model.0 <- lm(new_confirmed_cases ~ (ot1) , 
              data=who.new.cum.cases.time.sel.nonzero.gca)
model.1 <- lm(new_confirmed_cases ~ (ot1+ot2) , 
                data=who.new.cum.cases.time.sel.nonzero.gca)
model.2 <- lm(new_confirmed_cases ~ (ot1+ot2+ot3) , 
                data=who.new.cum.cases.time.sel.nonzero.gca)
model.3 <- lm(new_confirmed_cases ~ (ot1+ot2+ot3) +reporting_country_territory , 
                data=who.new.cum.cases.time.sel.nonzero.gca)
model.4 <- lm(new_confirmed_cases ~ (ot1+ot2+ot3) +reporting_country_territory
                +ot1:reporting_country_territory,
                data=who.new.cum.cases.time.sel.nonzero.gca)
model.5 <- lm(new_confirmed_cases ~ (ot1+ot2+ot3) +reporting_country_territory
                +ot1:reporting_country_territory +ot2:reporting_country_territory, 
                data=who.new.cum.cases.time.sel.nonzero.gca)
model.6 <- lm(new_confirmed_cases ~ (ot1+ot2+ot3) +reporting_country_territory
                +ot1:reporting_country_territory +ot2:reporting_country_territory
                +ot3:reporting_country_territory,
                data=who.new.cum.cases.time.sel.nonzero.gca)
anova(model.0, model.1, model.2, model.3, model.4, model.5, model.6)

# select parameters based on previous step:
model.sel.0 <- lm(new_confirmed_cases ~ (ot1) , 
              data=who.new.cum.cases.time.sel.nonzero.gca)
model.sel.1 <- lm(new_confirmed_cases ~ (ot1) +reporting_country_territory , 
              data=who.new.cum.cases.time.sel.nonzero.gca)
model.sel.2 <- lm(new_confirmed_cases ~ (ot1) +reporting_country_territory
              +ot1:reporting_country_territory,
              data=who.new.cum.cases.time.sel.nonzero.gca)
anova(model.sel.0, model.sel.1, model.sel.2)
summary(model.sel.0)
summary(model.sel.1)
summary(model.sel.2)
